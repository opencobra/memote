...
def abc_pump_formulae(base):
    ...
    pump.add_metabolites({aso_c: -1, atp: -1, h2o: -1, adp: 1, h: 1, pi: 1, aso_e: 1})
...
def phosphotransferase_system_formulae(base):
    ...
    malt6p = cobra.Metabolite("malt6p_c", formula="C12H21O14P", compartment="c")
...
def find_met_id(base):
    ...
    i.annotation = {
        "chebi": ["15378", "10744", "13357", "5584", "24636", "29233", "29234"]
    }
...
def compartments1(base):
    ...
    """
    test_case = {"default": "default", "c_15": "some unknown name"}
...
def compartment_size(base):
    ...
    """
    base.add_metabolites([cobra.Metabolite(i, compartment="c") for i in "ABCD"])
    base.add_metabolites([cobra.Metabolite(i, compartment="e") for i in "EFG"])
    base.add_metabolites([cobra.Metabolite(i, compartment="p") for i in "HI"])
    base.add_metabolites([cobra.Metabolite(i, compartment="v") for i in "JK"])
    return base
...
def compartment_size_uncommon_keys(base):
    ...
    Provide a model with different amounts metabolites for each compartment.
    """
    base.add_metabolites([cobra.Metabolite(i, compartment="ml") for i in "ABCD"])
    base.add_metabolites([cobra.Metabolite(i, compartment="om") for i in "EFG"])
    return base
...
def edge_case(base):
    ...
    """
    base.add_metabolites([cobra.Metabolite(i, compartment="ml") for i in "ABC"])
    base.add_metabolites([cobra.Metabolite(i, compartment="om") for i in "DEF"])
    return base
...
def test_find_met_in_model_accurate_results(model, mnx_id, compartment_id, expected):
    """Expect the metabolite represented by mnxid to be found correctly."""
    met = helpers.find_met_in_model(model, mnx_id, compartment_id=compartment_id)
    assert met[0].id == expected


@pytest.mark.parametrize(
    "model, mnx_id, compartment_id",
    [
        pytest.param(
            "find_met_incorrect_xref",
            "MNXM1",
            "c",
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            "no_compartments", "c", marks=pytest.mark.raises(exception=KeyError)
        ),
        pytest.param(
            "compartments1", "xx", marks=pytest.mark.raises(exception=KeyError)
        ),
    ],
    indirect=["model"],
)
...
@pytest.mark.parametrize(
    "model",
    [pytest.param("edge_case", marks=pytest.mark.raises(exception=RuntimeError))],
    indirect=["model"],
)
...
@pytest.mark.parametrize(
    "model, compartment_id, count",
    [("compartment_size", "c", 4), ("compartment_size", "h", 0)],
    indirect=["model"],
)
def test_find_metabolites_per_compartment(model, compartment_id, count):
    ...
    """
    assert len(helpers.metabolites_per_compartment(model, compartment_id)) == count


@pytest.mark.parametrize(
    "model, expected", [("compartment_size", "c")], indirect=["model"]
)
