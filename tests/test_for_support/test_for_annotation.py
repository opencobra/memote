...
def met_each_present(base):
    ...
    met.annotation = {
        ...
        "reactome": ["R-ALL-113557", "R-ALL-29398", "R-ALL-389680"],
        "bigg.metabolite": "pyr",
    }
    rxn = cobra.Reaction(id="RXN", name="Rxn")
...
def met_each_absent(base):
    met = cobra.Metabolite(id="met_c", name="Met")
    met.annotation = {"METANETX": "MNXM23", "old_database": "broken_identifier"}
    rxn = cobra.Reaction(id="RXN", name="Rxn")
    rxn.add_metabolites({met: -1})
    base.add_reactions([rxn])
    return base
...
def rxn_each_absent(base):
    ...
    rxn = cobra.Reaction(id="RXN", name="Rxn")
    rxn.annotation = {"old_database": "broken_identifier", "KEGG": "R01068"}
    base.add_reactions([rxn])
    return base
...
def gene_each_present(base):
    ...
    rxn.gene_reaction_rule = "gene1"
    base.add_reactions([rxn])
    base.genes[0].annotation = {
        "refseq": "YP_002410268.1",
        "uniprot": "P31663",
        "ecogene": "EG10173",
...
def inconsistent_ids(base):
    ...
    rxn4 = cobra.Reaction(id="KETOGLUCOSE-REDUCTASE-RXN", name="Reaction: 1.1.1.-")
...
@pytest.mark.parametrize(
    "model, num", [("met_each_present", 0), ("met_each_absent", 1)], indirect=["model"]
)
def test_generate_metabolite_annotation_overview(model, num, db):
    ...
    """
    overview = annotation.generate_component_annotation_overview(model.metabolites, db)
    assert len(overview) == num


@pytest.mark.parametrize("db", list(annotation.REACTION_ANNOTATIONS))
@pytest.mark.parametrize(
    "model, num", [("rxn_each_present", 0), ("rxn_each_absent", 1)], indirect=["model"]
)
def test_generate_reaction_annotation_overview(model, num, db):
    ...
    The required databases are outlined in `annotation.py`.
    """
    overview = annotation.generate_component_annotation_overview(model.reactions, db)
    assert len(overview) == num
...
@pytest.mark.parametrize(
    "model, num",
    [("gene_each_present", 0), ("gene_each_absent", 1)],
    indirect=["model"],
)
def test_generate_gene_annotation_overview(model, num, db):
    ...
    The required databases are outlined in `annotation.py`.
    """
    overview = annotation.generate_component_annotation_overview(model.genes, db)
    assert len(overview) == num
...
@pytest.mark.parametrize(
    "model, num, components",
    [("met_each_present", 0, "metabolites"), ("met_broken_id", 1, "metabolites")],
    indirect=["model"],
)
def test_generate_metabolite_annotation_miriam_match(model, num, components, db):
    """
...
@pytest.mark.parametrize(
    "model, num, components",
    [("rxn_each_present", 0, "reactions"), ("rxn_broken_id", 1, "reactions")],
    indirect=["model"],
)
...
@pytest.mark.parametrize(
    "model, num, components",
    [("gene_each_present", 0, "genes"), ("gene_broken_id", 1, "genes")],
    indirect=["model"],
)
...
@pytest.mark.parametrize(
    "model, namespace, num, components",
    [
...
def test_generate_component_id_namespace_overview(model, namespace, num, components):
    """Expect `num` component IDs to be from the same namespace."""
    overview = annotation.generate_component_id_namespace_overview(model, components)
