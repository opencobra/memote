...
def check_rxn_id_extracellular_tag(model):
    """
    Return a list of extracellular reactions in model
    that have not been tagged as such.
    ...
    reactions_with_all_metabolites_in_e = \
        [rxn for rxn in model.reactions if rxn.get_compartments() == list('e')]
    extracellular_reactions = (set(reactions_with_all_metabolites_in_e) -
                               set(find_demand_and_exchange_reactions(model)))
    return [rxn for rxn in extracellular_reactions
            if not re.match('[A-Z0-9]+.*?e.*?', rxn.id)]
...
def check_reaction_id_periplasm(model):
    ...
    model : model: cobra.core.Model.Model
            A cobrapy metabolic model
    """
    reactions_with_all_metabolites_in_pp = \
        [rxn for rxn in model.reactions if rxn.get_compartments() == list('p')]
    return [rxn for rxn in reactions_with_all_metabolites_in_pp
            if not re.match('[A-Z0-9]+.*?pp.*?', rxn.id)]


def check_reaction_tag_chloroplast(model):
    """
    Return a list of chloroplast reactions
    that have not been tagged as such.
    ...
    model : model: cobra.core.Model.Model
            A cobrapy metabolic model
    """
    reactions_with_all_mets_in_h = \
        [rxn for rxn in model.reactions if rxn.get_compartments() == list('h')]
    return [rxn for rxn in reactions_with_all_mets_in_h
            if not re.match('[A-Z0-9]+.*?h.*?', rxn.id)]
...
def check_reaction_tag_endoplasmatic_reticulum(model):
    ...
    model : model: cobra.core.Model.Model
            A cobrapy metabolic model
    """
    reactions_with_all_mets_in_er = []
    for rxn in model.reactions:
        if rxn.get_compartments() == list('er'):
            reactions_with_all_mets_in_er.append(rxn)

    return [rxn for rxn in reactions_with_all_mets_in_er
            if not re.match('[A-Z0-9]+.*?er.*?', rxn.id)]
...
def check_reaction_tag_golgi(model):
    ...
    model : model: cobra.core.Model.Model
            A cobrapy metabolic model
    """
    reactions_with_all_mets_in_g = \
        [rxn for rxn in model.reactions if rxn.get_compartments() == list('g')]
    return [rxn for rxn in reactions_with_all_mets_in_g
            if not re.match('[A-Z0-9]+.*?g.*?', rxn.id)]
...
def check_reaction_tag_lysosome(model):
    ...
    model : model: cobra.core.Model.Model
            A cobrapy metabolic model
    """
    reactions_with_all_mets_in_l = \
        [rxn for rxn in model.reactions if rxn.get_compartments() == list('l')]
    return [rxn for rxn in reactions_with_all_mets_in_l
            if not re.match('[A-Z0-9]+.*?l.*?', rxn.id)]
...
def check_reaction_tag_mitochondrion(model):
    ...
    model : model: cobra.core.Model.Model
            A cobrapy metabolic model
    """
    reactions_with_all_mets_in_m = \
        [rxn for rxn in model.reactions if rxn.get_compartments() == list('m')]
    return [rxn for rxn in reactions_with_all_mets_in_m
            if not re.match('[A-Z0-9]+.*?m.*?', rxn.id)]
...
def check_reaction_tag_nucleus(model):
    ...
    model : model: cobra.core.Model.Model
            A cobrapy metabolic model
    """
    reactions_with_all_mets_in_n = \
        [rxn for rxn in model.reactions if rxn.get_compartments() == list('n')]
    return [rxn for rxn in reactions_with_all_mets_in_n
            if not re.match('[A-Z0-9]+.*?n.*?', rxn.id)]
...
def check_reaction_tag_peroxisome(model):
    ...
    model : model: cobra.core.Model.Model
            A cobrapy metabolic model
    """
    reactions_with_all_mets_in_x = \
        [rxn for rxn in model.reactions if rxn.get_compartments() == list('x')]
    return [rxn for rxn in reactions_with_all_mets_in_x
            if not re.match('[A-Z0-9]+.*?x.*?', rxn.id)]
...
def check_reaction_tag_vacuole(model):
    ...
    model : model: cobra.core.Model.Model
            A cobrapy metabolic model
    """
    reactions_with_all_mets_in_v = \
        [rxn for rxn in model.reactions if rxn.get_compartments() == list('v')]
    return [rxn for rxn in reactions_with_all_mets_in_v
            if not re.match('[A-Z0-9]+.*?v.*?', rxn.id)]
